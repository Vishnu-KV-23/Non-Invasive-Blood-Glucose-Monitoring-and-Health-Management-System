...
#define BT_RX 10 // Connect to TXD pin of HC-05
#define BT_TX 11 // Connect to RXD pin of HC-05
...
void loop()
{
    ...
    if (millis() - tsLastReport > REPORTING_PERIOD_MS) {
        ...
        send_bluetooth_data(bpm, spo2, glucose_level);
    }
...
// Function to send data via HC-05 Bluetooth module
void send_bluetooth_data(int bpm, int spo2, float glucose_level) {
    // Create JSON-style formatted data
    BTSerial.print("{\"bpm\":");
    BTSerial.print(bpm);
    BTSerial.print(",\"spo2\":");
    BTSerial.print(spo2);
    BTSerial.print(",\"glucose\":");
    
    if (glucose_level > 500.0 || glucose_level < 0) {
        BTSerial.print("\"MEASURING\"");
    } else {
        BTSerial.print(glucose_level);
    }
    
    // Add historical data
    BTSerial.print(",\"history\":[");
    for (int i = 0; i < 3; i++) {
        BTSerial.print(glucose_records[i]);
        if (i < 2) BTSerial.print(",");
    }
    BTSerial.println("]}");
}
